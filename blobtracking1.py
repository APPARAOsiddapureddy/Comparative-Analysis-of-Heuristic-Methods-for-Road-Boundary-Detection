...
class VehicleTracker:
    def __init__(self, threshold=10):
        ...
        self.global_min_top = None
        self.global_max_bottom = None
    ...
    def get_min_max_coordinates(self):
        if not self.vehicle_dict:
            return (self.global_min_left, self.global_max_left, self.global_min_right, 
                    self.global_max_right, self.global_min_top, self.global_max_bottom)
        ...
        if self.global_min_top is None or min_top < self.global_min_top:
            self.global_min_top = min_top
        if self.global_max_bottom is None or max_bottom > self.global_max_bottom:
            self.global_max_bottom = max_bottom

        return (self.global_min_left, self.global_max_left, self.global_min_right, 
                self.global_max_right, self.global_min_top, self.global_max_bottom)

    def record_boundaries(self, frame_id):
        min_left, max_left, min_right, max_right, min_top, max_bottom = self.get_min_max_coordinates()
        self.frame_boundaries.append([frame_id, min_left, max_left, min_right, max_right, min_top, max_bottom])
        print(f"Frame {frame_id}: Min_Left = {min_left}, Max_Left = {max_left}, Min_Right = {min_right}, Max_Right = {max_right}, Min_Top = {min_top}, Max_Bottom = {max_bottom}")
...
def find_vehicle_boundaries(video_path, max_frames=250):
    ...
    cap.release()
    return tracker.frame_boundaries, tracker.road_boundaries, tracker.direction_count

def save_boundaries_to_csv(boundaries, road_boundaries, output_csv, direction_count):
...
def mark_vehicle_boundaries(video_path, output_path, output_csv, max_frames=250):
    boundaries, road_boundaries, direction_count = find_vehicle_boundaries(video_path, max_frames)
    if not boundaries and not road_boundaries:
        print("Error: No boundaries found. Ensure the video path is correct.")
        return

    save_boundaries_to_csv(boundaries, road_boundaries, output_csv, direction_count)
