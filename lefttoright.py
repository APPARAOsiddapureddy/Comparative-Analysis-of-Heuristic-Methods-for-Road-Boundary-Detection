...
def find_vehicle_boundaries(video_path, max_frames=250):
    ...
    # Return the top and bottom boundaries
    return tracker.boundaries_top, tracker.boundaries_bottom

def mark_vehicle_boundaries(video_path, min_max_boundaries, output_path, max_frames=1):
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        print("Error: Could not open video.")
        return
...
# Example usage:
video_path = 'footage_6.mp4' 
output_path = 'final_output.avi'

start_time = time.time()
min_max_boundaries = find_vehicle_boundaries(video_path)
end_time = time.time()
print(f"Time taken to find vehicle boundaries: {end_time - start_time:.2f} seconds")

start_time = time.time()
mark_vehicle_boundaries(video_path, min_max_boundaries, output_path)
end_time = time.time()
